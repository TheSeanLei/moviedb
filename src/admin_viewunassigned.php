...
#Create table that displays unassigned tickets
if (isset($_SESSION['username'])){
		$username = $_SESSION['username'];
}
$check = "SELECT tickets.Ticket_id, Received, Sender_Name, Sender_Email, Subject, Tech, Status, Select_Ticket FROM Tickets, ticket_to_admin WHERE Tickets.Ticket_id = ticket_to_admin.ticket_id AND ticket_to_admin.Tech = ''"; 


#display sorted by
if (isset($_POST['action']) && $_POST['action'] == 'Sort' && isset($_POST['sort'])) {
	$sortby = $_POST['sort'];
	$check = "SELECT tickets.Ticket_id, Received, Sender_Name, Sender_Email, Subject, Tech, Status, Select_Ticket FROM Tickets, ticket_to_admin WHERE Tickets.Ticket_id = ticket_to_admin.ticket_id AND ticket_to_admin.Tech = '' ORDER BY $sortby"; 
	}
...
#Display table data
$result = $db->query($check);
echo "<form action = 'admin_viewunassigned.php' method='POST'>";
